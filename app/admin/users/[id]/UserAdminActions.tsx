    ...
    <section className="rounded-xl border border-admin-line bg-admin-card">
      <header className="flex items-center justify-between border-b border-admin-line px-5 py-3">
        <h2 className="flex items-center gap-2 text-sm font-semibold">
          <Wallet2 className="h-4 w-4 text-zinc-600" />
          Balans və hesab idarəetməsi
        </h2>
        <div className="flex items-center gap-2">
          {dirty && (
            <span className="text-[11px] text-amber-700">Dəyişiklik var</span>
          )}
          <button
            type="button"
            onClick={save}
            disabled={pending || !dirty}
            className="inline-flex items-center gap-2 rounded-md bg-violet-500/10 px-3 py-1.5 text-xs font-semibold text-violet-700 ring-1 ring-violet-500/30 transition hover:bg-violet-500/20 disabled:opacity-50"
          >
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-admin-line pt-4">
          ...
          <button
            ...
            onClick={deleteUser}
            disabled={pending}
            className="inline-flex items-center gap-2 rounded-md bg-rose-500/10 px-3 py-1.5 text-xs font-semibold text-rose-700 ring-1 ring-rose-500/30 transition hover:bg-rose-500/20 disabled:opacity-50"
          >
  ...
  return (
    <label className="grid gap-1">
      ...
      <input
        ...
        inputMode="decimal"
        className="w-full rounded-md border border-admin-line bg-admin-card px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-600 focus:border-violet-500 focus:outline-none"
        placeholder="0.00"
      />
    </label>
  );
