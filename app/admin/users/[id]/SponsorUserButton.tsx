  ...
  if (isSponsored) {
    return (
      <button
        ...
        disabled={pending}
        className="inline-flex items-center gap-2 rounded-md bg-zinc-800 px-3 py-2 text-xs font-semibold text-zinc-300 ring-1 ring-zinc-700 transition hover:bg-zinc-700 disabled:opacity-50"
      >
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 rounded-md bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-200 ring-1 ring-amber-500/30 transition hover:bg-amber-500/20"
      >
        <Sparkles className="h-3.5 w-3.5" />
        Sponsorlu et
      </button>
      ...
      {open && (
        ...
        >
          ...
          <div
            ...
            className="relative w-full max-w-lg rounded-2xl border border-amber-500/30 bg-zinc-950 p-6 shadow-2xl"
          >
            <button
              type="button"
              onClick={() => !pending && setOpen(false)}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-lg font-bold text-zinc-100">Sponsorlu müştəri et</h3>
                <p className="mt-1 text-sm text-zinc-400">
                  Bu müştərinin referal linkiylə qeydiyyatdan keçən istifadəçilərin{" "}
                  <span className="font-semibold text-amber-200">oyun alışlarından</span>{" "}
                  standart faiz əvəzinə{" "}
                  <span className="font-semibold text-amber-200">{pct}%</span> komissiya bu
