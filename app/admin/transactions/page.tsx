      ...
      <div className="overflow-hidden rounded-xl border border-admin-line">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-admin-line">
            ...
            {txs.map((t) => (
              <tr key={t.id} className="hover:bg-admin-chip">
                <Td className="text-zinc-600">{fmtDate(t.createdAt)}</Td>
                <Td>
                  <Link
                ...
                <Td
                  className={`text-right font-medium ${
                    t.amountAznCents < 0 ? "text-rose-700" : "text-emerald-700"
                  }`}
                >
                  {t.amountAznCents < 0 ? "−" : "+"}
...
function TypeBadge({ type }: { type: string }) {
  const map: Record<string, string> = {
    DEPOSIT: "bg-fuchsia-500/15 text-fuchsia-700 ring-fuchsia-500/30",
    PURCHASE: "bg-violet-500/15 text-violet-700 ring-violet-500/30",
    COMMISSION: "bg-emerald-500/15 text-emerald-700 ring-emerald-500/30",
  };
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-medium ring-1 ${
        map[type] ?? "bg-admin-chip text-zinc-700 ring-admin-line2"
      }`}
    >
      {type}
    </span>
