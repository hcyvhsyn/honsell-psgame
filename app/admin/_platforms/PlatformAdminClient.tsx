      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            ...
            <span className="text-amber-700">Gözləmədə</span> olur — admin sifarişə əl ilə hesab
            məlumatlarını verəndə müştəriyə email göndərilir və profil panelində görünür.
          </p>
        </div>
        <button onClick={handleNew} className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white hover:bg-violet-500">
          ...
          <tbody className="divide-y divide-admin-line">
            {products.map((p) => {
              ...
              <tr key={p.id} className="transition hover:bg-admin-chip">
                ...
                <td className="px-5 py-4 font-medium text-zinc-800">{p.title}</td>
                {category === "AI" && (
                ...
                {category === "WORK" && (
                  <td className="px-5 py-4 text-zinc-700">
                    <div className="flex items-center gap-2">
                      ...
                      {meta.isPopular && (
                        <span className="rounded bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-bold uppercase text-amber-700">
                          Populyar
                        </span>
                      )}
                    </div>
                ...
                <td className="px-5 py-4">
                  {p.isActive ? (
                    <span className="rounded bg-emerald-500/15 px-2 py-0.5 text-xs text-emerald-700">Aktiv</span>
                  ) : (
                    <span className="rounded bg-rose-500/20 px-2 py-0.5 text-xs text-rose-600">Passiv</span>
                  )}
                </td>
          ...
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-admin-line bg-admin-card p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="block text-sm">
                ...
                <p className="mt-1 text-[11px] text-zinc-500">
                  Tövsiyə olunan ölçü: <b className="text-zinc-700">1200×900px</b> (4:3 aspekt) — platforma kartları public-də 4:3 nisbətdə render olunur.
                </p>
              </div>
            ...
            <div className="mt-8 flex justify-end gap-3">
              ...
              <button onClick={saveProduct} disabled={saving} className="rounded bg-violet-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">
