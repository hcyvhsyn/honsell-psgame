...
import { Check, Crown, ShoppingCart, TrendingDown, Zap } from "lucide-react";
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <div className="mt-4 flex flex-1 flex-col justify-end">
                  <button
                    ...
                    disabled={inCart}
                    onClick={() => addToCart(c)}
                    aria-label={inCart || isAdded ? "Səbətdədir" : "Səbətə əlavə et"}
                    className={`inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition-all duration-200 ${
                      inCart || isAdded
                        ...
                        : "bg-gradient-to-r from-violet-600 to-indigo-600 !text-white shadow-lg shadow-violet-600/20 hover:from-violet-500 hover:to-indigo-500 hover:shadow-violet-600/35"
                    }`}
                  >
                    {/* Mobildə yalnız ikon, sm-dən yuxarı ikon + yazı. Ekran
                        oxuyucusu üçün `aria-label` həmişə tam mətni verir. */}
                    {inCart || isAdded ? (
                      <>
                        <Check className="h-4 w-4 shrink-0" />
                        <span className="hidden sm:inline">Səbətdə</span>
                      </>
                    ) : (
                      <>
